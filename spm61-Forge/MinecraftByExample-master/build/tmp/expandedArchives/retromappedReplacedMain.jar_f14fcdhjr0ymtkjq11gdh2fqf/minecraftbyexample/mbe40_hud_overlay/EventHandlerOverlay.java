...
public class EventHandlerOverlay
{
  ...
  public void onEvent(RenderGameOverlayEvent.Pre event) {
    ...
    final int LAST_HOTBAR_SLOT_PLUS_ONE = FIRST_HOTBAR_SLOT + entityPlayerSP.field_71071_by.func_70451_h();
    for (int i = FIRST_HOTBAR_SLOT; i < LAST_HOTBAR_SLOT_PLUS_ONE; ++i) {
      ItemStack slotItemStack = entityPlayerSP.field_71071_by.func_70301_a(i);
      if (slotItemStack != null && slotItemStack.func_77973_b() == StartupCommon.itemHUDactivator) {
        foundInHotbar = true;
        break;
      }
    }
    ...
    switch (event.getType()) {
      case HEALTH:
        statusBarRenderer.renderStatusBar(event.getResolution().func_78326_a(), event.getResolution().func_78328_b());        /* Call a helper method so that this method stays organized */
