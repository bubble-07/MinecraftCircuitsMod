...
public class ContainerInventoryFurnace extends Container {
	...
	public ContainerInventoryFurnace(InventoryPlayer invPlayer, TileInventoryFurnace tileInventoryFurnace) {
		...
		for (int x = 0; x < HOTBAR_SLOT_COUNT; x++) {
			int slotNumber = x;
			func_75146_a(new Slot(invPlayer, slotNumber, HOTBAR_XPOS + SLOT_X_SPACING * x, HOTBAR_YPOS));
		}
		...
		for (int y = 0; y < PLAYER_INVENTORY_ROW_COUNT; y++) {
			for (int x = 0; x < PLAYER_INVENTORY_COLUMN_COUNT; x++) {
				...
				func_75146_a(new Slot(invPlayer, slotNumber,  xpos, ypos));
			}
		}
		...
		for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
			...
			func_75146_a(new SlotFuel(tileInventoryFurnace, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
		}
		...
		for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
			...
			func_75146_a(new SlotSmeltableInput(tileInventoryFurnace, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS+ SLOT_Y_SPACING * y));
		}
		...
		for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
			...
			func_75146_a(new SlotOutput(tileInventoryFurnace, slotNumber, OUTPUT_SLOTS_XPOS, OUTPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
		}
	}
	...
	{
		return tileInventoryFurnace.func_70300_a(player);
	}
	...
	public ItemStack func_82846_b(EntityPlayer player, int sourceSlotIndex)
	{
		...
		if (sourceSlotIndex >= VANILLA_FIRST_SLOT_INDEX && sourceSlotIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
			...
			if (TileInventoryFurnace.getSmeltingResultForItem(sourceStack) != null){
				if (!func_75135_a(sourceStack, FIRST_INPUT_SLOT_INDEX, FIRST_INPUT_SLOT_INDEX + INPUT_SLOTS_COUNT, false)){
					return null;
				}
			}	else if (TileInventoryFurnace.getItemBurnTime(sourceStack) > 0) {
				if (!func_75135_a(sourceStack, FIRST_FUEL_SLOT_INDEX, FIRST_FUEL_SLOT_INDEX + FUEL_SLOTS_COUNT, true)) {
	...
	public void func_75142_b() {
		...
		boolean fieldHasChanged [] = new boolean[tileInventoryFurnace.func_174890_g()];
		if (cachedFields == null) {
			cachedFields = new int[tileInventoryFurnace.func_174890_g()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i) {
			if (allFieldsHaveChanged || cachedFields[i] != tileInventoryFurnace.func_174887_a_(i)) {
				cachedFields[i] = tileInventoryFurnace.func_174887_a_(i);
    ...
    for (IContainerListener listener : this.field_75149_d) {
			for (int fieldID = 0; fieldID < tileInventoryFurnace.func_174890_g(); ++fieldID) {
				if (fieldHasChanged[fieldID]) {
					// Note that although sendProgressBarUpdate takes 2 ints on a server these are truncated to shorts
          listener.func_71112_a(this, fieldID, cachedFields[fieldID]);
				}
			}
		}
	}
	...
	public void func_75137_b(int id, int data) {
		tileInventoryFurnace.func_174885_b(id, data);
	}
